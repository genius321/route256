...
func (m *Manager) RunRepeatableRead(ctx context.Context, fn func(ctxTx context.Context) error) error {
	span, ctx := opentracing.StartSpanFromContext(ctx, "internal/repository/CancelOrder/tx/RunRepeatableRead")
	defer span.Finish()
...
func (m *Manager) RunSerializable(ctx context.Context, fn func(ctxTx context.Context) error) error {
	span, ctx := opentracing.StartSpanFromContext(ctx, "internal/repository/postgres/tx/RunSerializable")
	defer span.Finish()
	tx, err := m.pool.BeginTx(ctx, pgx.TxOptions{
